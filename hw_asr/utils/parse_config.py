...
class ConfigParser:
    def __init__(self, config, run_id=None):
        ...
        # write_json(self.config, self.save_dir / "config.yaml")
    ...
    def get_default_configs(cls):
        config_path = ROOT_PATH / "hw_asr" / "config.yaml"
        with config_path.open() as f:
            return cls(json.load(f))

    @classmethod
    def get_test_configs(cls):
        config_path = ROOT_PATH / "hw_asr" / "tests" / "config.yaml"
        with config_path.open() as f:
            return cls(json.load(f))
